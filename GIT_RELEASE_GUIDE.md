...
## VERSION 與 issue_list 撰寫準則（必遵守）

`VERSION.md` 是給使用者閱讀的版本摘要，請保持精簡。

- `VERSION.md` 只寫：
	- 新增/修改/刪除功能
	- 文字調整
	- BUG 修復
- `VERSION.md` 不寫過度細節（函式層級、逐行技術原因、長篇驗證過程）。
- 詳細技術內容、重現步驟、根因、驗證細節請寫在 `issue_list`。
- 若使用者直接要求你改某項功能或規則，完成後需將說明補到「最後一份 issue_list」（目前為編號最大檔案）末尾。
