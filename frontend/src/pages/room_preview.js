...
function formatPreviewCardSetSummary(mode) {
  const normalized = String(mode || 'all').trim();
  if (normalized === 'no_extend') return `${t('room.info.card_pool')}:[B]`;
  if (normalized === 'expansion_only') return `${t('room.info.card_pool')}:[E]`;
  return `${t('room.info.card_pool')}:[B]+[E]`;
}

function formatPreviewToggleSummary(value) {
  return value ? t('room.info.setting_on') : t('room.info.setting_off');
}

function formatPreviewRoomSettingsSummary(room) {
  const boomTimeoutMinutes = Number(room?.turn_timeout_minutes || 3);
  return [
    `${t('room.info.trip_rule')}:${formatPreviewToggleSummary(Boolean(room?.require_trip))}`,
    formatPreviewCardSetSummary(room?.expansion_mode || 'all'),
    `${t('room.info.initial_green_card')}:${formatPreviewToggleSummary(Boolean(room?.enable_initial_green_card))}`,
...
export function renderVillageInfo({ el, esc, withVillageSuffix, goToRegisterPage, state }, data) {
  ...
  const roomSettings = formatPreviewRoomSettingsSummary(room);
  ...
  el.villageInfoList.innerHTML = `
    ...
    <li class="village-info-row village-info-row-meta">
      ...
      <span class="village-info-item"><strong>${esc(t('room.info.count'))}</strong>${Number(room.player_count || 0)}/${maxPlayers} <span class="village-info-settings">(${esc(roomSettings)})</span></span>
    </li>
...
function renderSystemStage({ el, esc }, data, state) {
  if (!el.systemMessages) return;
  
  const isReplayView = isReplayViewState(state, data);
  const messages = Array.isArray(data?.chat_messages) ? data.chat_messages : [];
  const systemMsgs = messages.filter((msg) => String(msg?.type || '').toLowerCase() === 'system');
  ...
  const timeoutWarnMsg = (status === 2 && Number.isFinite(timeoutRemain))
    ? `[${fmtTime(Math.floor(Date.now() / 1000))}] ${t('room.info.turn_timeout')}${t('room.info.turn_timeout_fmt', { who: timeoutCurrent || '-', n: Math.max(0, timeoutRemain) })}`
    : null;
  const boomedWarnMsg = (status === 2 && boomedName)
    ? `[${fmtTime(Math.floor(Date.now() / 1000))}] ${t('room.info.boomed_notice')}${t('room.info.boomed_notice_fmt', { name: boomedName })}`
    : null;
  ...
  systemMsgs.forEach((msg) => {
    ...
    if (!text) return;
    htmlLines.push(`<div class="system-line">[${fmtTime(ts)}] ${esc(text)}</div>`);
  });

  if (timeoutWarnMsg) {
    htmlLines.push(`<div class="system-line system-timeout">${esc(timeoutWarnMsg)}</div>`);
  }
  if (boomedWarnMsg) {
    htmlLines.push(`<div class="system-line system-boomed">${esc(boomedWarnMsg)}</div>`);
  }

  if (!htmlLines.length) {
    el.systemMessages.innerHTML = `<p class="lighttxt">${esc(t('room.system.empty'))}</p>`;
  } else {
    el.systemMessages.innerHTML = htmlLines.join('');
  }
...
function renderChatStage({ el, esc }, data, state) {
  ...
  const messages = Array.isArray(data?.chat_messages) ? data.chat_messages : [];
  // Only show chat messages (type === 'chat'), filter out system messages
  const chatMsgs = messages.filter((msg) => String(msg?.type || '').toLowerCase() !== 'system');
...
export function renderState({
  ...
  if (el.systemMessages) {
    renderSystemStage({ el, esc }, data, state);
  }
