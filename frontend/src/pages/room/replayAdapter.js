  ...
  if (hasStructuredFinalStatePlayers) {
    ...
    return {
      room: {
        ...
        require_trip: parseBool(record?.game_settings?.require_trip),
        expansion_mode: String(record?.game_settings?.expansion_mode || 'all'),
        enable_initial_green_card: parseBool(record?.game_settings?.enable_initial_green_card),
        enable_neutral_chaos_mode: parseBool(record?.game_settings?.enable_neutral_chaos_mode),
        turn_timeout_minutes: parseTimeoutMinutes(record?.game_settings?.turn_timeout_minutes),
