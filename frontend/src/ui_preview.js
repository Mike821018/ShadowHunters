      ...
      return `
        <article class="${classes}" data-player-account="${esc(account)}" style="border-left:3px solid ${cardColor}" aria-label="${esc(t('ui.player_label', { name: p.name || account }))}">
          <header class="player-card-head">
            ...
            <div class="player-color-cell" style="flex-shrink:0; margin-left:auto;">${colorControlHtml}</div>
          </header>
          <div class="player-card-meta">
            <span>📍 ${esc(areaDisplayName)}</span>
            <span style="margin-left:auto;">
              <button class="player-role-trigger" type="button" aria-label="${esc(t('ui.role_detail'))}" style="border:none; background:none; padding:0; font-size:inherit;">
                ${roleNameEn ? `<span class="damage-meter-badge ${esc(roleCampClass)}" style="margin-right:4px;">${esc(roleBadgeText)}</span>` : ''}
                <span class="player-role-name">${esc(roleDisplayName)}</span>
              </button>
              ${roleTooltip ? `
                <div class="player-role-popover" role="tooltip" style="display:none;">
